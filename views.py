...
def index():
	...
	if data['name'].split('.')[-1]!='mp3':
		...
		if data['command']!="send":
			...
			audio = Audio_Base(username="S7bhash",name=data["name"],uname=filepath)
...
def files():
	files = Image_Base.query.filter_by(username="Dhananjay")
			...
			if upload.file.data.filename.split('.')[-1]!='mp3':
				...
				for ip in hostnames:
					if ip != "5000":
						...
						command="scjhhsacjbds"
						dict = {'name':upload.name.data,"command":command,"image":peer_audio[i],"shape":shape}
						shape = (1,2,3)
						reliable_send(ip,dict)
					else:
						rand = ''.join([random.choice(string.ascii_letters) for i in range(32)])
